...
// "ვინ ვართ" გვერდზე Organization schema ყველაზე რელევანტურია.
// Google-ს ეხმარება Knowledge Panel-ის ასაგებად (მარჯვენა panel ძიებაში).
function OrganizationJsonLd({ t }: { t: AboutUsData }) {
  ...
  const schema = {
    ...
    sameAs: [
      'https://facebook.com/itechno',
      'https://instagram.com/itechno',
      'https://linkedin.com/company/itechno',
    ],
  }
...
// whyUs სექცია = კომპანიის უპირატესობები, ItemList-ად structured data-ში
function WhyUsJsonLd({ t }: { t: AboutUsData }) {
  const schema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: t.whyUs.title,
    numberOfItems: t.whyUs.items.length,
    itemListElement: t.whyUs.items.map((item, idx) => ({
      '@type': 'ListItem',
      position: idx + 1,
      name: item.title,
  ...
  return (
    <>
      ...
      <div className="bg-white text-[#0F172A] antialiased">
        ...
        <section className="max-w-[1440px] mx-auto pt-12 sm:pb-24 px-6 grid lg:grid-cols-2 gap-20 items-center">
          ...
          <div className="relative h-[520px] rounded-[30px] overflow-hidden shadow-2xl">
            <Image
              ...
              alt={`${data.hero.titleBlue} ${data.hero.titleBlack}`}
              fill
        ...
        {/* PRIORITY SECTION */}
        <section
          className="bg-white py-20 px-6 border-y border-slate-200"
          aria-label={data.priority.title}
        >
        ...
        {/* SUPPORT SECTION */}
        <section className="bg-white py-10 px-6" aria-label={data.support.badge}>
        ...
        {/* WHY US SECTION */}
        <section className="bg-[#F8FAFC] py-20 px-6" aria-label={data.whyUs.badge}>
          <div className="max-w-[1440px] mx-auto">
            <div className="grid lg:grid-cols-12 gap-14 items-start">
              ...
              <div className="lg:col-span-8">
                {/* dl/dt/dd — სემანტიკური HTML სიისთვის, crawlers-ისთვის სწორი სტრუქტურა */}
